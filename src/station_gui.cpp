...
static const NWidgetPart _nested_station_view_widgets[] = {
	...
	NWidget(NWID_HORIZONTAL, NC_EQUALSIZE),
		...
		NWidget(WWT_PUSHTXTBTN, COLOUR_GREY, WID_SV_CARGO_FROM), SetMinimalSize(60, 12), SetResize(1, 0), SetFill(1, 1),
...
struct StationViewWindow : public Window {
	...
	StationViewWindow(const WindowDesc *desc, WindowNumber window_number) : Window()
	{
		...
		this->GetWidget<NWidgetCore>(WID_SV_CARGO_FROM)->SetDataTip(StationViewWindow::last_cargo_from_str, StationViewWindow::last_cargo_from_tooltip);
	...
	virtual void OnPaint()
	{
		...
		NWidgetCore *cargo_btn = this->GetWidget<NWidgetCore>(WID_SV_CARGO_FROM);
	...
	void HandleCargoWaitingClick(int row)
	{
		...
		bool dest_view = this->GetWidget<NWidgetCore>(WID_SV_CARGO_FROM)->widget_data != STR_STATION_VIEW_WAITING_TO_BUTTON;
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (widget) {
			...
			case WID_SV_CARGO_FROM: {
				...
				NWidgetCore *nwi = this->GetWidget<NWidgetCore>(WID_SV_CARGO_FROM);
				...
				this->SetWidgetDirty(WID_SV_CARGO_FROM);
				this->SetWidgetDirty(WID_SV_WAITING);
				this->SetWidgetDirty(WID_SV_SCROLLBAR);
				break;
			}
